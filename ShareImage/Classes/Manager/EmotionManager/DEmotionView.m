...
//  DFrame
//
//  Created by DaiSuke on 2017/1/18.
//  Copyright © 2017年 DaiSuke. All rights reserved.
//

#import "DEmotionView.h"




@implementation DEmotion
@synthesize image = _image;

+ (instancetype)emotionWithIdentifier:(NSString *)identifier displayName:(NSString *)displayName{
    DEmotion *emotion = [[DEmotion alloc] init];
...
- (UIImage *)image{
    if (!_image) {
        _image = [UIImage getEmojiImageWithName:self.identifier];
    }
    return _image;
}
...
@class DEmotionPageView;

@protocol DEmotionPageViewDelegate <NSObject>

@optional
- (void)emotionPageView:(DEmotionPageView *)emotionPageView didSelectEmotion:(DEmotion *)emotion atIndex:(NSInteger)index;
- (void)didSelectDeleteButtonInEmotionPageView:(DEmotionPageView *)emotionPageView;

@end

/// 表情面板每一页的cell，在drawRect里将所有表情绘制上去，同时自带一个末尾的删除按钮
@interface DEmotionPageView : UICollectionViewCell

@property(nonatomic, weak) DEmotionView<DEmotionPageViewDelegate> *delegate;
...
/// 分配给当前pageView的所有表情
@property(nonatomic, copy) NSArray<DEmotion *> *emotions;
...
@implementation DEmotionPageView
...
- (void)drawRect:(CGRect)rect {
    ...
    for (NSInteger i = 0, l = self.emotions.count; i < l; i++) {
        ...
        DEmotion *emotion = self.emotions[i];
...
- (void)handleDeleteButtonEvent:(UIButton *)deleteButton {
...
@interface DEmotionView ()<DEmotionPageViewDelegate>

@property(nonatomic, strong) NSMutableArray<NSArray<DEmotion *> *> *pagedEmotions;
@property(nonatomic, assign) BOOL debug;
@end

@implementation DEmotionView
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        [self.collectionView registerClass:[DEmotionPageView class] forCellWithReuseIdentifier:@"page"];
...
- (void)setEmotions:(NSArray<DEmotion *> *)emotions {
    _emotions = emotions;
    [self pageEmotions];
}
...
- (void)layoutSubviews {
    ...
    self.pageControl.frame = CGRectMake(0, CGRectGetHeight(self.bounds) - self.pageControlMarginBottom - pageControlHeight - 10, CGRectGetWidth(self.bounds), pageControlHeight);
    
    self.bottomView.frame = CGRectMake(0, CGRectGetMaxY(self.pageControl.frame), CGRectGetWidth(self.bounds), 30);
    
    self.sendButton.frame = CGRectMake(CGRectGetWidth(self.bounds) - 40, 0, 40, 30);
}
...
- (void)pageEmotions {
    ...
    if (!CGRectIsEmpty(self.collectionView.bounds) && self.emotions.count && !CGSizeIsEmpty(self.emotionSize)) {
        ...
        for (NSInteger i = 0; i < pageCount; i ++) {
            ...
            NSArray<DEmotion *> *emotionForPage = [self.emotions objectsAtIndexes:[NSIndexSet indexSetWithIndexesInRange:emotionRangeForPage]];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    DEmotionPageView *pageView = [collectionView dequeueReusableCellWithReuseIdentifier:@"page" forIndexPath:indexPath];
    ...
    [pageView.deleteButton setImage:[self.deleteButtonImage imageWithAlpha:0.5f] forState:UIControlStateHighlighted];
...
#pragma mark - <DEmotionPageViewDelegate>
...
@interface DEmotionView (UIAppearance)

@end

@implementation DEmotionView (UIAppearance)
...
+ (void)setDefaultAppearance {
    DEmotionView *appearance = [DEmotionView appearance];
    appearance.backgroundColor = [UIColor whiteColor];
    appearance.deleteButtonImage = [UIImage getEmojiImageWithName:@"emotion_delete"];
    ...
    appearance.sendButtonTitleAttributes = @{NSFontAttributeName: [UIFont systemFontOfSize:13], NSForegroundColorAttributeName: [UIColor whiteColor]};
    appearance.sendButtonBackgroundColor = [UIColor blueColor];
...
//    appearance.sendButtonMargins = UIEdgeInsetsMake(0, 0, 16, 16);
    ...
    UIPageControl *pageControlAppearance = [UIPageControl appearanceWhenContainedIn:[DEmotionView class], nil];
    pageControlAppearance.pageIndicatorTintColor = DColor(210, 210, 210);
    pageControlAppearance.currentPageIndicatorTintColor = DColor(162, 162, 162);
}
