...
static NSArray<DEmotion *> *QQEmotionArray;

@protocol DQQEmotionInputViewProtocol <UITextInput>

@property(nonatomic, copy) NSString *text;
@property(nonatomic, assign, readonly) NSRange selectedRange;
@end

@implementation DEmotionManager
- (instancetype)init {
    self = [super init];
    if (self) {
        _emotionView = [[DEmotionView alloc] init];
        self.emotionView.emotions = [DEmotionManager emotionsForQQ];
        __weak DEmotionManager *weakSelf = self;
        self.emotionView.didSelectEmotionBlock = ^(NSInteger index, DEmotion *emotion) {
...
- (UIView<DQQEmotionInputViewProtocol> *)boundInputView {
    if (self.boundTextField) {
        return (UIView<DQQEmotionInputViewProtocol> *)self.boundTextField;
    } else if (self.boundTextView) {
        return (UIView<DQQEmotionInputViewProtocol> *)self.boundTextView;
    }
    return nil;
}
...
+ (UIImage *)imageForQQEmotionWithIdentifier:(NSString *)identifier {
    return [UIImage getEmojiImageWithName:identifier];
}

+ (NSArray<DEmotion *> *)emotionsForQQ {
    if (QQEmotionArray) {
        return QQEmotionArray;
    }
    
    NSMutableArray<DEmotion *> *emotions = [[NSMutableArray alloc] init];
    ...
    for (NSString *emotionString in emotionStringArray) {
        ...
        DEmotion *emotion = [DEmotion emotionWithIdentifier:identifier displayName:emotionItem.lastObject];
...
// 在子线程预加载
+ (void)asyncLoadImages:(NSArray<DEmotion *> *)emotions {
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        for (DEmotion *e in emotions) {
            [e image];
        }
    });
}
