...
/**
 ...
 [_textField setFrame:0 y:100 w:self.view.width h:30];
 ...
 *  @warning 一个`DEmotionManager`无法同时绑定`boundTextField`和`boundTextView`，在两者都绑定的情况下，优先使用`boundTextField`。
 *  @warning 由于`DEmotionManager`里面多个地方会调用`boundTextView.text`，而`setText:`并不会触发`UITextViewDelegate`的`textViewDidChange:`或`UITextViewTextDidChangeNotification`，从而在刷新表情面板里的发送按钮的enabled状态时可能不及时，所以`DEmotionManager`要求绑定的`QMUITextView`必须打开`shouldResponseToProgrammaticallyTextChanges`属性
 */

@interface DEmotionManager : NSObject
/// 要绑定的UITextField
 ...
 */
@property(nonatomic, strong, readonly) DEmotionView *emotionView;
