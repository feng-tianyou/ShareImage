...
/**
 当前表情展示出来的名字，可用于输入框里的展位文字，例如"[委屈]"
 */
@property (nonatomic, copy) NSString *displayName;
/**
 表情对应的图片，若表情图片存放于项目内，则建议当前表情的`identifier`作为图片名
 */
@property (nonatomic, strong) UIImage *image;

/**
 快速生成一个`Emotion`对象，并且以`identifier`为图片名在当前项目里查找，作为表情的图片
 ...
 *
 *  本控件支持通过`UIAppearance`设置全局的默认样式。若要修改控件内的`UIPageControl`的样式，可通过`[UIPageControl appearanceWhenContainedIn:[DEmotionView class], nil]`的方式来修改。
 *
 */
@interface DEmotionView : UIView<UICollectionViewDataSource, UICollectionViewDelegateFlowLayout>
 ...
 *  @argv  emotion 被选中的表情对应的`DEmotion`对象
 *  @see DEmotion
 */
@property (nonatomic, copy) void (^didSelectEmotionBlock)(NSInteger index, DEmotion *emotion);
...
/// 表情的图片大小，不管`DEmotion.image.size`多大，都会被缩放到`emotionSize`里显示，默认为{30, 30}
