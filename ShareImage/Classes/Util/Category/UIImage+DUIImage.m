...
- (UIImage *)imageByRoundCornerRadiusWithimageViewSize:(CGSize)imageViewSize{
    ...
    CGSize size = imageViewSize.width > 0 ? imageViewSize : self.size;
    CGFloat radius = MAX(size.width, size.height) / 2.0;
    ...
    UIBezierPath *path = [UIBezierPath bezierPathWithRoundedRect:CGRectInset(rect, 0, 0) byRoundingCorners:UIRectCornerAllCorners cornerRadii:CGSizeMake(radius, 0)];
    [path closePath];
