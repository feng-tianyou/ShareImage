...
- (void)logoutByType:(LogoutType)type{
    ...
    NSString *btnTitle = @"";
    switch (type) {
        case LogoutTypeForChangeDevice:{
            strContent = kVIEW_TEXT_LOGOUT_CHANGE_DEVICE;
            break;
        }
        case LogoutTypeForNoValid:{
            strContent = kVIEW_TEXT_LOGOUT_NOVALID;
            break;
        }
        case LogoutTypeForNoVerifyCode:{
            strContent = kVIEW_TEXT_LOGOUT_NOVERIFYCODE;
            break;
        }
        case LogoutTypeForNormal:{
            strContent = kVIEW_TEXT_LOGOUT_NORMAL;
            break;
        }
        case LogoutTypeForEditPassword:{
            strContent = kVIEW_TEXT_LOGOUT_EDIT_PSW;
            btnTitle = @"去登录";
        }
            break;
        default:
...
//    if([KGLOBALINFOMANAGER getIsAlertLogout] || [[self class] isSubclassOfClass:[TGLoginView class]]){
//        return;
//    }
    [KGLOBALINFOMANAGER setIsAlertLogout:YES];
    
    DAlertView *alertView = [[DAlertView alloc] initWithTitle:@"" andMessage:strContent];
    __weak UIViewController *weakSelf = self;
    if(type == LogoutTypeForNormal){
        [alertView addButtonWithTitle:@"" type:CustomAlertViewButtonTypeCancel handler:^(DAlertView *alertView) {
            [KGLOBALINFOMANAGER setIsAlertLogout:NO];
        }];
    }
    [alertView addButtonWithTitle:btnTitle type:CustomAlertViewButtonTypeDefault handler:^(DAlertView *alertView) {
//        [[TGUserNetwork shareEngine] clearRefreshTokenBlockCache];
        [DCacheManager setCacheObjectByData:nil forKey:kVIEW_KEY_LOGOUT];
        if(type == LogoutTypeForNormal){
            NetworkStatus networkStatus = [[Reachability reachabilityForInternetConnection]
                                           currentReachabilityStatus];
            if (networkStatus != NotReachable) {
                
                NSString *strClass = NSStringFromClass([self class]);
                NSDictionary *dicUserInfo = @{KVIEWNAME:strClass};
                [self addNetworkLoadingViewByText:@"正在退出" userInfo:dicUserInfo];
//                [[TGUserNetwork shareEngine] postLogoutOnSucceeded:^(NSDictionary *dic) {
//                    [weakSelf removeNetworkLoadingView];
//                    [weakSelf logout];
